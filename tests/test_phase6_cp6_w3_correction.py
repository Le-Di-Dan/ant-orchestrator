"""CP6 Window 3 correction tests (§6.1–§6.8).

Verifies that ALL settled attempt statuses (SUCCEEDED, FAILED, PERMANENT_FAILURE,
TRANSIENT, DEADLINE, ISOLATION) are recovered from durable authority without calling
the backend again. Prior to this correction only SUCCEEDED was protected.

§6.1  Deterministic failure Window 3 → no backend rerun, correct route.
§6.2  Transient failure Window 3 → outcome recovered; new attempt only after routing.
§6.3  Plain deadline Window 3 → recovered, no backend rerun.
§6.4  Permanent failure variants → recovered, no backend rerun.
§6.5  Cancellation in Window 3 context (attempt-level; workflow-level in §4.6).
§6.6  Missing/corrupt compact JSON → fail-closed, no backend rerun.
§6.7  Concurrent recover same settled attempt → same outcome, no duplicate rows.
§6.8  Regression: SUCCEEDED still recovered (no regression from correction).
"""
...
def _settle_attempt(
    orch: AttemptOrchestrator,
    run_id: str,
    logical_action_id: str,
    outcome: TestExecutionOutcome,
) -> str:
    """Simulate: before_execute + after_execute (with compact JSON) for a given outcome."""
    attempt_id = orch.before_execute(run_id, logical_action_id)
    compact = _to_compact_json(outcome)
    orch.after_execute(attempt_id, outcome.outcome, compact_json=compact)
    return attempt_id


# ---------------------------------------------------------------------------
# §6.1 — deterministic failure Window 3
# ---------------------------------------------------------------------------


def test_w3_deterministic_failure_no_backend_rerun(tmp_path: Path) -> None:
    """§6.1: settled PERMANENT_FAILURE → backend not called on replay."""
    clock = _clock()
    db = _make_db(tmp_path, clock)
    orch = _make_orch(db, clock)
    det_outcome = _failed_outcome(attempt_ref="att-1")
    original_id = _settle_attempt(orch, "R1", "T1-test", det_outcome)
    ...
    assert recovered.outcome is WorkerOutcome.PERMANENT_FAILURE
    assert recovered.disposition is RecoveryDisposition.TERMINAL_FAILED


# ---------------------------------------------------------------------------
# §6.2 — transient failure Window 3 (recover before routing)
# ---------------------------------------------------------------------------


def test_w3_transient_failure_outcome_recovered(tmp_path: Path) -> None:
    """§6.2: settled RETRYABLE_FAILURE → outcome recovered, no backend rerun, no new attempt."""
    clock = _clock()
    db = _make_db(tmp_path, clock)
    orch = _make_orch(db, clock)
    original_id = _settle_attempt(orch, "R1", "T1-test", _transient_outcome("att-1"))
    ...
    assert _count_rows(db, "execution_attempts") == 1
    assert recovered.attempt_ref == original_id
    assert recovered.outcome is WorkerOutcome.RETRYABLE_FAILURE
    assert recovered.disposition is RecoveryDisposition.RETRY
    assert recovered.reason_code is TestReasonCode.TRANSIENT_INTERRUPTION_AUDITED


# ---------------------------------------------------------------------------
# §6.3 — deadline Window 3
# ---------------------------------------------------------------------------


def test_w3_deadline_no_backend_rerun(tmp_path: Path) -> None:
    """§6.3: settled deadline failure → no backend rerun, escalation route."""
    clock = _clock()
    db = _make_db(tmp_path, clock)
    orch = _make_orch(db, clock)
    deadline_outcome = _failed_outcome(
        ...
        worker_outcome=WorkerOutcome.ESCALATION,
        attempt_ref="att-1",
    )
    _settle_attempt(orch, "R1", "T1-test", deadline_outcome)
...
# ---------------------------------------------------------------------------
# §6.4 — permanent failure variants
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize(
    ...
    [
        (TestReasonCode.EXECUTABLE_MISSING, FailureCategory.EXECUTABLE_MISSING),
        (TestReasonCode.PERMISSION_DENIED, FailureCategory.PERMISSION_DENIED),
        (TestReasonCode.COMMAND_POLICY_DENIED, FailureCategory.POLICY_VIOLATION),
        (TestReasonCode.ISOLATION_SETUP_FAILED, FailureCategory.ISOLATION_SETUP_FAILURE),
    ],
)
def test_w3_permanent_variants_no_backend(
    tmp_path: Path, reason_code: TestReasonCode, category: FailureCategory
) -> None:
    """§6.4: all permanent failure variants recovered without backend rerun."""
    clock = _clock()
    db = _make_db(tmp_path, clock)
    orch = _make_orch(db, clock)
    perm_outcome = _failed_outcome(reason_code=reason_code, category=category, attempt_ref="att-1")
    _settle_attempt(orch, "R1", "T1-test", perm_outcome)
    ...
    assert ant.calls == 0
    assert recovered.reason_code is reason_code
    assert _count_rows(db, "execution_attempts") == 1
...
def test_w3_null_compact_json_failed_escalates(tmp_path: Path) -> None:
    ...
    # Simulate legacy settle: after_execute without compact_json (null outcome column).
    ...
    assert ant.calls == 0  # backend never called
    assert _count_rows(db, "execution_attempts") == 1  # no new attempt
    assert recovered.outcome is WorkerOutcome.ESCALATION
    assert recovered.disposition is RecoveryDisposition.ESCALATE
...
def test_w3_corrupt_compact_json_escalates(tmp_path: Path) -> None:
    ...
    attempt_id = orch.before_execute("R1", "T1-test")
    # Store corrupt JSON directly via UoW (simulating data corruption).

...
### CP5 (commit feat(workers): add durable documentation ant execution)
- **Composer boundary provider-neutral**: port `application/ports/documentation_composer.py` (`DocumentationComposer`, `ComposerContext`/`ComposerSource`, `CompositionConstraints`, `CompositionResult`) chỉ phụ thuộc `DocumentationTask`/`ModelCompositionDraft`/`ModelUsage` — không type provider/SDK. `ComposerContext` mang sources đã verify (không để port import `context/`). Production composer `LLMDocumentationComposer` dùng `LLMAdapter`; raw response bị parser tiêu thụ rồi loại bỏ, không persist/log.
- **Provider idempotency capability (trung thực)**: `LLMAdapter.complete` **không** có idempotency key/reconciliation/status-query. Do đó receipt `INVOKING` mơ hồ → `IN_DOUBT`, fail closed, **không** auto-retry, không tự suy đoán provider chưa chạy. Không over-claim exactly-once. `RESERVED` (provider chưa được gọi vì `INVOKING` persist ngay trước call) → an toàn resume fresh.
- **Composition receipt = bridge provider-phase** (`workers/documentation/receipt.py`, schema v1, checksummed, atomic): `RESERVED→INVOKING→COMPLETED→ENERGY_SETTLED` (monotonic) + terminal `FAILED`/`IN_DOUBT`. Tách trách nhiệm với mutation journal (FS-phase) của CP4; recovery ordering: journal tồn tại → ưu tiên `MutationRecovery` (no provider, digest-link draft↔proposed bắt buộc khớp). Draft artifact + receipt nằm dưới `composition/` của attempt root, **tái dùng** atomic writer CP4 (thêm 2 `ArtifactKind`, không duplicate writer).
- **Energy lifecycle (CP5 in-process)**: port `application/ports/worker_energy.py` + impl `energy/worker_lifecycle.py` (in-memory, deterministic, production-capable; ledger/UoW thật là CP6). reverify trước provider; settle sau, idempotent theo `invocation_id` (no double-charge khi recovery); usage `UNAVAILABLE` → conservative fallback = reserved (không zero); actual > reserved → `OVER_BUDGET`, settle nhưng **không** mutate. Durability "đã settle" do receipt `ENERGY_SETTLED` mang.
- **Async bridge** (`infrastructure/async_bridge.py`): một event-loop boundary, timeout/cancellation, cleanup task/loop, **không** nested `asyncio.run`, **không** leak. Active-loop guard raise **trước** khi tạo coroutine ⇒ provider call count 0. Timeout/cancel → sanitized error, không auto-retry.
- **Report authoritative (§12)**: `files_read` từ context manifest, `files_changed` từ `MutationResult`, `commands=()` (CP5 no shell), `result` system-derived; chỉ `summary`/`risks`/`next_steps` lấy sanitized từ draft. Operational field do model khai (target/operation/files/...) bị parser reject (`ProhibitedModelFieldError`) — adversarial tests. Không raw key echo trong error.
- **Context store thêm `load()`**: `ContextPackageStore.verify()` ủy quyền cho `load()` (verify + trả `LoadedSource`); cùng đường digest-check, không nới lỏng fail-closed.
- **Biên CP5 = journal `PUBLISHED`**: không persist DB WorkerRun/Evidence, không graph wiring, không `COMPLETED`, không bump `WORKFLOW_DEFINITION_VERSION`, không live provider. Run_id truyền explicit vào `execute(task, scope, run_id)` (scope CP1 frozen không mang run_id) — CP6 wiring cấp từ GraphState.

### CP6 (commit feat(workflows): integrate durable documentation ant execution)
- **PO-locked decisions**: KHÔNG migration DB; idempotency = deterministic IDs + PK uniqueness + compare-and-verify (không silent overwrite); rich refs trong typed/versioned/bounded sanitized JSON envelope ở cột `execution_evidence.result`; WorkerRun/Evidence/energy commit chung 1 SQLite UoW; giữ `GRAPH_STATE_SCHEMA_VERSION=2`; bump `WORKFLOW_DEFINITION_VERSION` 2→3; field state CP6 optional (schema-2 round-trip) nhưng fail-closed trên production path.
- **Deterministic identity chain** (`integration/identity.py`): `run→logical_action→proposal_digest→attempt → worker_run_id/evidence_id/energy_settlement_id`; `invocation_id` khớp đúng công thức nội bộ Ant (energy settlement gắn cùng provider invocation). PK = idempotency authority: retry trùng ID = no-op/verify, authority khác = structured conflict fail-closed.
- **Connection-bound repos** (`persistence/repositories/execution_records.py`) + UoW expose `worker_runs/evidence/energy_usage`: 3 record persist atomic trong 1 transaction (`Conn*Repository`, `find` cho reconcile). Phase-1 `Sqlite*Repository` (own-transaction) giữ nguyên.
- **Durable energy** (`energy/durable_lifecycle.py` `DurableEnergyLifecycle` + `energy/settlement_math.py` dùng chung): production KHÔNG dùng `InMemoryEnergyLifecycle`; durable record = row `energy_usage` ghi trong UoW từ receipt facts; idempotent theo invocation (no double-charge); over-budget persist trung thực nhưng không success.
- **Ordering §11** (`integration/persistence_mapper.py`): Ant trả authoritative result → verify receipt/journal/target digest → assemble envelope → 1 UoW persist+reconcile WorkerRun/Evidence/energy → commit → mới `journal PUBLISHED→COMPLETED` → mới settle attempt. DB fail → journal giữ PUBLISHED, attempt chưa success, recovery idempotent không gọi provider/không republish. COMPLETED+DB missing = invariant violation fail-closed (kiểm trước khi reconcile).
- **Execution adapter** (`integration/execution_adapter.py`, graph-facing port `application/ports/documentation_execution.py`): load+digest-verify proposal → stable attempt (`AttemptOrchestrator.before_execute`) → seed durable energy → assemble immutable scope → Ant (provider-neutral; Ant tự recover draft, no 2nd provider) → persist+complete+settle. Recovery window-4 verified: reuse attempt, provider call 0, no duplicate record. Controlled failure (IN_DOUBT/over-budget/...) → no journal complete, no attempt success.
- **Proposal/context pre-approval** (`integration/proposal_store.py` + `documentation_preparer.py`): `ContextPreparationService` (CP2 thật) build context + `ExecutionProposal`, persist bundle (proposal+task) làm artifact JSON, set `proposal_ref/digest/context_package_ref/manifest_digest` + action_intent vào initial state TRƯỚC approval (RunWorkflow additive, gated; legacy không đổi). Proposal digest tamper → fail-closed.
- **Approval binding** (`workflows/graph_support.build_escalation_payload` + `graph._bind_approval`): payload escalation mang proposal_ref/digest/manifest_digest; resume APPROVE→EXECUTE verify `payload.proposal_digest == state.proposal_digest` (mismatch → `approval_binding_mismatch` fail-closed) + stamp `approval_ref`. Chỉ kích hoạt khi có proposal (Phase 4 không đổi).
- **Graph rewiring** (`workflows/graph.py` + `runner.py`): inject `documentation_execution` port (optional); execute node dùng port khi có (no stub fallback; thiếu proposal → `missing_proposal_authority` fail-closed), giữ stub qua DI cho legacy/unit test. State thêm proposal_ref/proposal_digest/approval_ref (optional, JSON-safe; no-pickle giữ nguyên).
- **Composition** (`integration/composition.py build_documentation_execution` + `cli/workflow_composition.py`): factory provider-neutral, `DurableEnergyLifecycle`, fail-closed khi thiếu composer; `build_workflow_services` nhận optional `documentation_execution`/`documentation_preparer` (default None → Phase 4 stub không đổi). Live-provider CLI wiring + full vertical E2E thuộc CP7 (CP6 dùng deterministic fake composer cho gate).
- **Version guard**: bump 2→3 làm checkpoint definition-2 fail-closed (`WorkflowDefinitionMismatch`); fixture `tests/support/cp8_driver.py` chuyển dùng hằng số `WORKFLOW_DEFINITION_VERSION` thay literal 2 (không silent-migrate checkpoint thật).
- **Biên CP6 = path wired + verified với fake composer**: KHÔNG live provider, KHÔNG full fixture vertical-slice acceptance matrix (CP7). Generic-Task→DocumentationRequest mapping cấp qua injected `request_factory` (CP7 fixture brief cung cấp cụ thể).

### CP7 (commit test(e2e): verify phase 5 documentation ant vertical slice)
- **Fixture harness** (`tests/support/cp7_fixture.py`): dựng FULL production stack trên temp Nest — bootstrap SQLite + real `BoundedFileSystemAdapter` (path-policy/redaction/audit) + `ContextPackageBuilder` thật + `DurableEnergyLifecycle` + proposal/approval binding + execution adapter + LangGraph runner, wired qua `build_workflow_services(documentation_execution=, documentation_preparer=)`. Test drive `create_task → run_workflow → resolve_approval.approve` đúng như CLI; fixture source không bị mutate (mỗi run temp riêng).
- **Vertical slice E2E** (`tests/test_phase5_cp7_e2e.py`): CREATE + UPDATE qua real human-approval interrupt + durable resume → 1 provider call, document tạo/replace đủ required sections, 1 WorkerRun + 1 Evidence + 1 energy row, journal COMPLETED, attempt SUCCEEDED, task COMPLETED; no raw provider body trong DB. Evidence-bundle test verify target_digest == sha256(published), mọi artifact ref (journal/receipt/proposed/before/diff) tồn tại dưới artifact root, refs repo-relative (no host abs path).
- **Restart/adversarial**: window-1 (re-enter trước approval idempotent, provider 0, approval không tái tạo) + post-completion replay (`WorkflowStateError`, no dup) ở E2E; window-2/3/4 covered ở CP6 adapter-level (recovery window-4: reuse attempt, provider 0, no dup) + CP5 ant-level. Out-of-scope target → permission denied trước provider (provider 0, no WorkerRun). Proposal-tamper/approval-binding-mismatch covered CP6.
- **Provider-backed Ollama** (`tests/test_phase5_cp7_ollama.py`, NON-DEFAULT, gated `ANT_OLLAMA_E2E=1`): live `qwen2.5-coder:7b` qua LiteLLM/Ollama seam → SAME vertical slice PASS, published 894-char handoff đủ sections, no raw output persist. Prompt (`workers/documentation/prompt.py`) làm rõ: raw JSON only (no code fences) + `proposed_content` là single Markdown string với `## ` section headings — prompt-engineering, KHÔNG relax parser/security (typed-JSON contract + template id/version giữ nguyên; model vẫn non-authoritative).
- **Environment evidence**: host THIẾU symlink capability (Windows no Developer Mode/admin) → symlink-escape/alias E2E **BLOCKED** honestly (skip với reason, không biến skip thành PASS, không hạ security guarantee). Ollama AVAILABLE → live slice PASS. Context-store concurrency: Phase 5 production = single-process (ProposalStore/ContextPackageStore persist immutable theo identity; same-id+same-digest reuse, khác-digest conflict) — không xây distributed-lock framework.
- **Biên CP7**: KHÔNG live provider trong default regression (fake composer là quality gate); KHÔNG bắt đầu CP8.

...
### CP6 — Factory, composition, boundary regression, live verification, closure — IMPLEMENTED; LIVE CLOSURE PASS
- `adapters/factory.py`: `build_llm_adapter(endpoint, *, secret_provider, completion_client) -> LLMAdapter`. Branching explicit, tối thiểu, audit được — **không** registry/dynamic-import/DI-container/fallback/router/cache/singleton. Mapping: `provider == "openai"` → `LiteLLMCloudAdapter`; `provider == "ollama"` → `OllamaAdapter`. Provider khác (gồm `fake`) → **reject** bằng `ConfigInvalid` **trước** mọi client call (tái dùng error config-layer, **không** mở rộng LLM invocation taxonomy). Provider so khớp **verbatim** — không strip/lowercase/auto-correct. Factory chỉ construct: **không** network, **không** secret lookup, **không** ping.
- **Composition semantics:** `cli/composition.py` thêm `ConfiguredLLMAdapters(queen: LLMAdapter | None, local: LLMAdapter | None)` (frozen DTO) + `build_configured_llm_adapters(config) -> ConfiguredLLMAdapters`, wiring production `EnvSecretProvider` + `LiteLLMSdkClient` vào factory. Endpoint vắng → slot `None` (config Phase 1 không có `models` ⇒ hai slot `None`, không lỗi). Role **không** ép provider — factory quyết định từ `endpoint.provider`. `build_services` (Phase 1) giữ nguyên signature/behavior. **Không** eager network/secret lookup, **không** dead `asyncio.run` bridge (Phase 2 chưa có CLI command gọi model).
- **Boundary audit (mở rộng):** thêm `test_production_source_does_not_import_tests` (toàn `src` không import `tests.*`) + `test_factory_imports_only_allowed_internal_modules` (factory chỉ import adapter cloud/local/client + application.ports + config; **không** import test double). Các rule CP3/CP5 (litellm chỉ trong adapters; no openai/ollama SDK; no real-execution lib; core/application/config infra-free) giữ nguyên.
- **Live verification process:** test gated `@pytest.mark.live_openai` / `@pytest.mark.live_ollama`, skip mặc định; chạy thật qua public `LLMAdapter.complete`; assert provider/model khớp endpoint, `is_local`, finish reason hợp lệ, usage MEASURED|UNAVAILABLE; **không** log prompt/output/secret/base_url. Preflight chỉ kiểm tra PRESENT/MISSING của `OPENAI_API_KEY`/`ANT_LIVE_OPENAI_MODEL`/`ANT_LIVE_OLLAMA_MODEL`/`ANT_LIVE_OLLAMA_BASE_URL` — không in value.
- **Blocker policy (§XIII):** thiếu env / live skip / live fail ⇒ **không** tuyên bố closure PASS, **không** Phase 2 COMPLETE, **không** ROADMAP COMPLETED, **không** tạo completion report PASS giả; automated (factory/composition/tests/boundary) vẫn hoàn thành + commit được; verdict `CP6 IMPLEMENTATION PASS — LIVE CLOSURE BLOCKED`. Khi env có sau: chạy live → tạo/cập nhật completion report → amend CP6 nếu chưa push.
- **Closure evidence path:** `docs/plans/PHASE_2_COMPLETION_REPORT.md` — **đã tạo** với closure PASS (automated + cả hai live PASS, sanitized evidence).
- **Live closure thực tế:** live OpenAI **PASS** + live Ollama **PASS** (qua public `LLMAdapter.complete`, sanitized evidence trong completion report). `.env.live` (secret) ignored+untracked qua `.gitignore` policy `.env`/`.env.*` với `!.env.example`/`!.env.template`. CP6 amend thành closure commit (message `feat(adapters): wire providers and close phase 2`). ROADMAP **chưa** đổi COMPLETE — chờ PO ra lệnh merge/closure riêng.
